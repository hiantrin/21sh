...
/*   Updated: 2020/02/23 20:43:42 by hiantrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			test_dir(char *argv, int *status, t_pi pi)
{
	...
	if (new == NULL)
	{
		ft_putstr_fd("21sh: cd: no such file or directory: ", pi.pipe[2][1]);
	...
	{
		ft_putstr_fd("21sh: cd: not a directory: ", pi.pipe[2][1]);
		ft_putendl_fd(argv, pi.pipe[2][1]);
		status[0] = 0;
		free(new);
		return (0);
...
{
	ft_putendl_fd("21sh: cd: Too many arguments.", pipe[2][1]);
	status[0] = 0;
}
